...
void set_mode(int width, int height)
{
    ...
    screen = SDL_SetVideoMode(width*scale,height*scale, 16, SDL_HWSURFACE|SDL_DOUBLEBUF|(fullscreen?SDL_FULLSCREEN:0));
    if ( !screen )
    {
        printf("%s\n",SDL_GetError());
        die(-1,0);
...
void kbd_emulate_gamepad (void)
{
	...
	if (keyboard_mod[0] & 2) 
		gamepad_buttons[0] |= gamepad_X;
	if (keyboard_key[0][0] == 44)
		gamepad_buttons[0] |= gamepad_Y;
		
	if (keyboard_key[0][0] == 43)
		gamepad_buttons[0] |= gamepad_L;
		
	if (keyboard_key[0][0] == 42)
		gamepad_buttons[0] |= gamepad_R;

	if (keyboard_key[0][0] == 40)
		gamepad_buttons[0] |= gamepad_start;*/
		
		
	//printf("keyboard_key[0][0] %d\n", keyboard_key[0][0]);
